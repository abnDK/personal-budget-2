...
export class CategoryRow implements Category {
    name: string;
    amount: number;
    id: number;
    parent_id: number;
    ...
    readValuesFromDomElement(): void {
        ...
        if (this.frozen) {
            ...
            const name: string =
                this.dom_element_ref.querySelector(".category-name").innerText;

            const amount: number = Number(
                this.dom_element_ref.querySelector(".category-amount").innerText
            );
            ...
            const name: string =
                this.dom_element_ref.querySelector(".category-name").value;

            const amount: number = Number(
                this.dom_element_ref.querySelector(".category-amount").value
            );
...
export class Budget {
    ...
    constructor(rows: Category[], budgetRowsDomElement: Element) {
        ...
        this.id =
            parseInt(window.location.href.split("/").at(-1)) ??
            "unknowns budget_id";
        ...
        if (state) {
            ...
            */
            ...
            this.deleteCategoryRows()
                .then(() => {
                    console.log(
                        "After deleting categories in object and db: ",
                        this.rows
                    );
                ...
                .then(() => {
                    console.log(
                        "After updating categories with name, amount and parent_ids: ",
                        this.rows
                    );
                    console.log("Status: AWAITING");
                ...
                .finally(() => {
                    console.log(
                        "After rendering budget to the DOM: ",
                        this.rows
                    );
                    console.log("Status: SUCCESS");
                });
    ...
    get loners() {
        ...
        const parentIdsOfChildren = Array.from(
            new Set(this.children.map((child) => child.parent_id))
        );

        return this.parents.filter(
            (row) => !parentIdsOfChildren.includes(row.id)
        );
    }
    ...
    renderNewRow = (newRow: CategoryRow): void => {
        ...
        const indexOfParent = childrenArray.findIndex(
            (categoryRow) => categoryRow.dataset.id == newRow.parent_id
        );
    ...
    deleteCategoryRows = async (): Promise<void> => {
        for (const deletableRow of this.toDelete) {
            ...
            await this.handleTransactionCategoryForeignKeyConstraint(
                deletableRow.id,
                deletableRow.parent_id
            );

            await this.handleCategoryParentIdForeignKeyConstraint(
                deletableRow.id,
                deletableRow.parent_id
            );
    ...
    updateCategoryRows = async () => {
        ...
        const categoriesParentIds = await this.query.getCategoriesParentIds(
            this.root.budget_id
        );
        ...
        for (const cat of this.rows) {
            ...
            await this.query.updateCategoryNameAmount(
                cat.id,
                cat.name,
                cat.amount
            );
    ...
    handleTransactionCategoryForeignKeyConstraint = async (
        oldCategoryId: number,
        newCategoryId: number
    ): Promise<void> => {
        const transactionsWithCategoryId =
            await this.query.getTransactionsByCategoryId(oldCategoryId);

        for (const transaction of transactionsWithCategoryId) {
            await this.query.updateCategoryIdOfTransaction(
                transaction.id,
                newCategoryId
            );
        }
    ...
    handleCategoryParentIdForeignKeyConstraint = async (
        categoryIdToBeDeleted: number,
        newParentId: number
    ): Promise<void> => {
        const childrenOfDeletedCategory = await this.query.getCategoryChildren(
            categoryIdToBeDeleted
        );

        for (const categoryChild of childrenOfDeletedCategory) {
            try {
                await this.query.updateCategoryParentId(
                    categoryChild.id,
                    newParentId
                );
            } catch (err) {
    ...
    */
    ...
    renderBudgetSum = (): void => {
        document.querySelector(
            ".budget-sum"
        ).innerText = `Budget sum: ${this.sum}`;
    };
}

export class BudgetPage {
    static renderTitles = (): void => {
        document.querySelector(".budget-title").innerHTML += `${
            PERIOD.monthNames[PERIOD.MONTH]
        } ${PERIOD.YEAR}`;
        document.querySelector(".transactions-header-title").innerHTML += `${
            PERIOD.monthNames[PERIOD.MONTH]
        } ${PERIOD.YEAR}`;
    };
}
