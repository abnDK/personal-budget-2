...
class TransactionService {
    // TEST FUNCTION. 
    static getTransactionsPromise() {
        // get Transactions in database
        let data = pool.query('SELECT * FROM transaction ORDER BY id ASC');
        // build array of transactions
        data.then(data => {
            console.log(data);
            let trans = data.rows.map(res => new transaction_1.Transaction(parseInt(res.id), res.name, res.amount, res.date));
            console.log(trans);
            return trans;
        }, error => {
            return error;
    ...
    static getTransactions() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            // build array of transactions
            let transactions = data.rows.map(res => new transaction_1.Transaction(parseInt(res.id), res.name, res.amount, res.date, res.category_id));
            return transactions;
        });
    }
    static getTransactionById(id) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let transaction_in_array = data.rows.map(res => new transaction_1.Transaction(parseInt(res.id), res.name, res.amount, res.date));
    ...
    static createTransaction(name, amount, date, category_id) {
        return __awaiter(this, void 0, void 0, function* () {
            let category;
            ...
            // create transaction
            let data_trans = yield pool.query('INSERT INTO transaction (name, amount, date, category_id) VALUES ($1, $2, $3, $4) RETURNING *', [name, amount, date, category_id]);
